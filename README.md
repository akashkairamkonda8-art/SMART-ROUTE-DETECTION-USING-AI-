...
Rather than subjective arbitrary labels, the congestion class is derived from a reproducible, deterministic Congestion Index (
) calculated using the Speed Ratio and Volume-to-Capacity (
) ratio:
With additional sensitivity to road incident/construction blockages:
The continuous 
 is discretized into 4 standard Level of Service congestion categories:
0 = Low Congestion (
): Free-flow conditions, average speed 
 of speed limit, minimal queuing.
1 = Moderate Congestion (
): Stable flow, speeds at 
 of speed limit, minor junction delays.
2 = High Congestion (
): Unstable traffic flow, noticeable stop-and-go waves, significant delay.
3 = Severe Congestion (
): Flow breakdown, severe queuing, gridlock conditions, travel time more than doubled.
